...
export async function load({ params }) {
	...
	const subtaskCount = await prisma.tasks.count({
		where: {
			parentTask: params.taskID
		}
	})
	
	return { user, subject, workspace, workspaceMembers, board, task, subtasks, members, chats, chatChatSenders, viewers, createdBy, statuses, anAdmin, subtaskCount };
}
...
export const actions = {
	...
	taskNewLevel: async ({ request, params }) => {
		...
		const newlog = await prisma.logs.create({
			data: {
				commiter: cUser.id,
				log: `set the task ${toUpdateTask.name} to ${newLevel == 1 ? 'low' : newLevel == 2 ? 'medium' : 'high'} level/priority in workspace ${workspace.name}`,
				logDate: new Date(),
				type: 'task',
				involve: workspace.members
			}
	...
	taskSetFav: async ({ request, params }) => {
		...
		if (!task) return invalid(404, { message: 'Task not found please try again', reason: 'databaseError' })
		
		// create a log for the current user
		const newlog = await prisma.logs.create({
			data: {
				commiter: cUser.id,
				log: `${setFav === 'set' ? 'set' : 'remove'} task ${task.name} as ${cUser.gender === 'Male' ? 'his' : 'her'} favorite task`,
				logDate: new Date(),
				type: 'task',
				involve: [cUser.id]
			}
	...
	taskSetNewStatus: async ({ request, params }) => {
		...
		if(!board) return invalid(500, {message: 'Failed to update receiving board', reason: 'databaseError'})
	...
	taskAddAssignee: async ({ request, params }) => {
		...
		if(!newMember) return invalid(404, {message: 'Member not found please try again later', reason: 'databaseError'})
	...
	taskRemAssignee: async ({ request, params }) => {
		...
		if(!rUser) return invalid(404, {message: 'Member not found please try again later', reason: 'databaseError'})
	...
	taskAddSubtask: async ({ request, params }) => {
		...
		if (!pTask) return invalid(404, { message: 'Parent task not found please try again later', reason: 'databaseError' })
