<script>
  ...
  let newTaskName = data.subtask.name
  ...
  */
  ...
  onMount(() => {
    ...
    activeTask.set(data.parentTask)
    activeSubtask.set(data.subtask)
    $breadCrumbsItems = [
      ...
      {text: data.parentTask.name, href: `/${data.user.email}/${data.subject.id}/${data.workspace.id}/${data.board.id}`},
      {text: data.subtask.name, href: `/${data.user.email}/${data.subject.id}/${data.workspace.id}/${data.board.id}/${data.parentTask.id}`},
      {text: 'view', href: '#'}
    ]
    oldDescriptionValue = data.subtask.description
      ...
      <div class="columns is-mobile is-multiline">
        ...
        <div style='{innerWidth < 769 ? 'border-bottom: 1px solid rgba(0, 0, 0, 0.3); margin-bottom: 3%' : ''}' class="column is-12-mobile is-flex is-justify-content-space-between is-align-items-center">
          ...
          <div class="is-flex is-flex-direction-column {taskNameEditing ? '' : 'is-hidden'}">
            <div class='{newTaskName !== data.subtask.name ? '' : 'is-hidden'}'>
              <Button icon>
        ...
        <div class="column is-narrow-tablet is-3-mobile centerxy is-relative">
          ...
          <div
            ...
            <div class="tag m-0 is-{data.subtask.level == 3 ? 'danger' : data.subtask.level == 2 ? 'warning' : 'success'}">
              Priority
            </div>
            <div class="tag m-0 is-{data.subtask.level == 3 ? 'danger' : data.subtask.level == 2 ? 'warning' : 'success'} is-light is-relative">
              {data.subtask.level == 3 ? 'High' : data.subtask.level == 2 ? 'Medium' : 'Low'}
          ...
          <div
            ...
            on:click={() => showLevels = !showLevels}
            class="is-hidden-tablet is-clickable tag is-{data.subtask.level == 3 ? 'danger' : data.subtask.level == 2 ? 'warning' : 'success'} is-relative">
            {data.subtask.level == 3 ? 'High' : data.subtask.level == 2 ? 'Medium' : 'Low'}
        ...
        <div class="column is-narrow-tablet is-3-mobile centerxy is-relative">
          ...
          <div
            ...
            <div class="tag m-0 is-warning is-light is-relative">
              {#if data.user.favorites[2].ids.includes(data.subtask.id)}
                True
              {:else}
                False
              {/if}
            ...
            <div class="tag m-0 is-info is-light is-relative">
              ...
              <div
                ...
                <div class='p-3 has-text-grey-dark fredoka-reg'>
                  There {data.subtask.subtasks.length > 1 ? 'are' : 'is'} {data.subtask.subtasks.length} subtask{data.subtask.subtasks.length > 1 ? 's' : ''}
                </div>
              </div>
            </div>
          </div>
            ...
            <Window value={currentWindow}>
              ...
              <WindowItem style='border: 1px solid rgba(0, 0, 0, 0.3)' class='maxmins-h-48v is-relative rounded-b'>
                <Textarea color='grey' autofocus autogrow rows={17} bind:value={description} class='maxmins-h-48v pl-3 fredoka-reg txt-size-{innerWidth < 571 ? '13' : ''}'>
                  {data.subtask.description ? '' : 'Description'}
                </Textarea>
              ...
              <WindowItem style='border: 1px solid rgba(0, 0, 0, 0.3)' class='maxmins-h-48v rounded-b'>
                <div class="maxmins-w-100p maxmins-h-48v is-relative">
                  ...
                  {#each data.sub2Tasks as sub2task}
                    <a href="{sub2task.id}">
                      <div
                        on:mouseenter={() => subtaskHovering(sub2task.id, true)}
                        on:mouseleave={() => subtaskHovering(sub2task.id, false)}
                        style='border-bottom: 1px solid rgba(0, 0, 0, 0.3)' class='maxmins-w-100p is-flex is-align-items-center has-transition px-2 py-{innerWidth < 571 ? '1' : '2'} {subtaskHoverings && currentSubtask === sub2task.id ? 'has-background-grey-lighter' : 'has-background-white'}'
                      >
                        <!-- subtask name -->
                        ...
                        <div class="tags mb-0 has-addons">
                          ...
                          <div class='tag mb-0 fredoka-reg is-info'>
                            {sub2task.level == 1 ? 'L' : sub2task.level == 2 ? 'M' : 'H'}
                          </div>
  
                          <div class='tag mb-0 fredoka-reg is-{data.statuses.filter(status => status.id === sub2task.status)[0].color}'>
                            {data.statuses.filter(status => status.id === sub2task.status)[0].name}
                          </div>
                        </div>
                      </div>
                    </a>
